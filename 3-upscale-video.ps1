...
#ls -Filter '1/*' | % { $a = split-path $_ -LeafBase; cmd /c "vspipe -c y4m --arg ""in=$_"" --arg is_img=False --arg model=301 upscale_and_rife_2.vpy - | ffmpeg -hide_banner -y -i - -i ""$_"" -map 0:v -map 1:a? -map 1:s? -c:a copy -c:s copy -c:v hevc_nvenc -preset p7 -pix_fmt p010le -profile:v main10 -b:v 0K -cq 26 ""./3/$a.mkv""" }
...
ffmpeg -v warning -stats -y -hwaccel d3d11va -ss (Get-Random -Maximum ($duration-60)) -t "00:00:01" -i $a -ss (Get-Random -Maximum ($duration-60)) -t "00:00:01" -i $a -ss (Get-Random -Maximum ($duration-60)) -t "00:00:01" -i $a -ss (Get-Random -Maximum ($duration-60)) -t "00:00:01" -i $a -ss (Get-Random -Maximum ($duration-60)) -t "00:00:01" -i $a -ss (Get-Random -Maximum ($duration-60)) -t "00:00:01" -i $a -filter_complex '[0:v][0:a][1:v][1:a][2:v][2:a][3:v][3:a][4:v][4:a][5:v][5:a]concat=n=6:v=1:a=1[v][a]' -map '[v]' -map '[a]' -c:a flac -fps_mode passthrough -c:v hevc_nvenc -preset p7 -tune lossless -pix_fmt p010le -profile:v main10 2/test.mkv
if (2160 -gt $height){
    cmd /c "vspipe -c y4m --arg in=2/test.mkv --arg is_img=False --arg model=5010 upscale_and_rife_2.vpy - | ffmpeg -v warning -stats -y -i - -map 0:v -c:v hevc_nvenc -preset p7 -tune lossless -pix_fmt p010le -profile:v main10 2/test2.mkv"
    cmd /c "vspipe -c y4m --arg in=2/test.mkv --arg is_img=False --arg model=301 upscale_and_rife_2.vpy - | ffmpeg -v warning -stats -y -i - -map 0:v -c:v hevc_nvenc -preset p7 -tune lossless -pix_fmt p010le -profile:v main10 2/test_301.mkv"
}
else{
