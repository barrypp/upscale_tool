...
#ls -Filter '../1/*' | % { $a = split-path $_ -LeafBase; cmd /c "vspipe -c y4m --arg ""in=$_"" --arg is_img=False --arg model=5010 upscale_and_rife_2.vpy - | ffmpeg -hide_banner -y -i - -i ""$_"" -map 0:v -map 1:a? -map 1:s? -c:a copy -c:s copy -c:v hevc_nvenc -preset p7 -pix_fmt p010le -profile:v main10 -b:v 0K -cq 28 ""../3/$a.mkv""" }
...
if (2160 -gt $height){
    cmd /c "vspipe -c y4m --arg in=tmp2/test.mkv --arg is_img=False --arg model=5010 upscale_and_rife_2.vpy - | ffmpeg -v warning -stats -y -i - -map 0:v -c:v hevc_nvenc -preset p7 -tune lossless -pix_fmt p010le -profile:v main10 tmp2/test2.mkv"
    cmd /c "vspipe -c y4m --arg in=tmp2/test.mkv --arg is_img=False --arg model=301 upscale_and_rife_2.vpy - | ffmpeg -v warning -stats -y -i - -map 0:v -c:v hevc_nvenc -preset p7 -tune lossless -pix_fmt p010le -profile:v main10 tmp2/test_301.mkv"
}
else{
    mv tmp2/test.mkv tmp2/test2.mkv
}
ffmpeg -v warning -stats -y -i tmp2/test2.mkv -map 0:v -c:v hevc_nvenc -preset p7 -pix_fmt p010le -profile:v main10 -b:v 0K -cq 26 tmp3/p7_cq26.mkv
ffmpeg -v warning -stats -y -i tmp2/test2.mkv -map 0:v -c:v hevc_nvenc -preset p7 -pix_fmt p010le -profile:v main10 -b:v 0K -cq 28 tmp3/p7_cq28.mkv
#ffmpeg -v warning -stats -y -i tmp2/test2.mkv -map 0:v -c:v libx265 -crf 18 -preset slow tmp3/slow_crf18.mkv
...
if ($true){
    ...
    ls tmp3/*.mkv | sort | ForEach-Object {
        ...
        ffmpeg -v warning -stats -i $_ -i tmp2/test2.mkv -lavfi "[0:v]setpts=PTS-STARTPTS[dis];[1:v]setpts=PTS-STARTPTS[ref];[dis][ref]libvmaf=n_threads=8'$($model):log_fmt=csv:log_path=tmp3/$($_.name).vmaf.csv" -f null -
