...
ls -Filter '1/*' | ForEach-Object {
    ...
    cmd /c "vspipe -c y4m --arg ""in=$s/$a.mkv"" --arg is_img=False upscale_and_rife_2.vpy - | ffmpeg -hide_banner -y -i - -i ""$s/$a.mkv"" -map 0:v -map 1 -map -1:v -c:a copy -c:s copy -c:v hevc_nvenc -preset p7 -pix_fmt p010le -profile:v main10 -b:v 0K ""./3/$a.mkv"""
}
