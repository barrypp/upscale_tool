...
def ESRGAN_NV(
	input : vs.VideoNode,
	lt_hd : bool = False,
	model : typing.Literal[0, 2, 5005, 5006, 5007, 5008, 5009, 5010,301,302] = 5005,
	gpu : typing.Literal[0, 1, 2] = 0,
	gpu_t : int = 2,
	st_eng : bool = False,
	ws_size : int = 0,
	...
	if not isinstance(lt_hd, bool) :
		raise vs.Error(f"模块 {func_name} 的子参数 lt_hd 的值无效")
	if model not in [0, 2, 5005, 5006, 5007, 5008, 5009, 5010,301,302] :
		raise vs.Error(f"模块 {func_name} 的子参数 model 的值无效")
	if gpu not in [0, 1, 2] :
		raise vs.Error(f"模块 {func_name} 的子参数 gpu 的值无效")
	if not isinstance(gpu_t, int) or gpu_t <= 0 :
	...
	plg_dir = os.path.dirname(core.trt.Version()["path"]).decode()
	mdl_fname = ["RealESRGANv2-animevideo-xsx2", "realesr-animevideov3", "animejanaiV2L1", "animejanaiV2L2", "animejanaiV2L3", "animejanaiV3-HD-L1", "animejanaiV3-HD-L2", "animejanaiV3-HD-L3","RealESRGAN_x2plus","RealESRGAN_x4plus"][[0, 2, 5005, 5006, 5007, 5008, 5009, 5010,301,302].index(model)]
	mdl_pth = plg_dir + "/models/RealESRGANv2/" + mdl_fname + ".onnx"
	if not os.path.exists(mdl_pth) :
		raise vs.Error(f"模块 {func_name} 所请求的模型缺失")
	...
	cut1 = core.resize.Bilinear(clip=input, format=vs.RGBH, matrix_in_s="709" if not is_img else None)
	cut2 = vsmlrt.RealESRGANv2(clip=cut1, scale=4 if model==2 or model==302 else 2, model=model, backend=vsmlrt.BackendV2.TRT(
		num_streams=gpu_t, force_fp16=True, output_format=1,
		workspace=None if ws_size < 128 else (ws_size if st_eng else ws_size * 2),
		use_cuda_graph=True, use_cublas=False, use_cudnn=False,
		static_shape=st_eng, min_shapes=[0, 0] if st_eng else [64, 64],
